...
const SimpleMentionInput = () => {
  ...
  useEffect(() => {
    // in Android, text is before selection
    // therefore, update mention range using curr selection and prev text
    if (!!clickRange) {
      setClickRange(null);
    } else {
      updateMentionRange(prevText, selection);
    }
  }, [text]);
